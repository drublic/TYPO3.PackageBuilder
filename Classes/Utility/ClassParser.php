...
class ClassParser {
	...
	/**
	 * @var \TYPO3\PackageBuilder\Domain\Model\ClassObject\ClassObject
	 */
	...
	protected $currentMethod = array(
		'reflection' => NULL,
		// the Tx_ExtensionBuilder_Reflection_MethodReflection returned from ClassReflection'methodObject' => NULL,
		// the new created Tx_ExtensionBuilder_Domain_Model_Class_Method'endline' => 0,'methodBody' => ''
	);
	...
	protected function initClassObject($className) {
		$this->classObject = new \TYPO3\PackageBuilder\Domain\Model\ClassObject\ClassObject($className);
		$this->classReflection = new \TYPO3\PackageBuilder\Reflection\ClassReflection($className);
...
' . $this->concatLinesFromArray($this->lines, $this->multiLinePropertyMatches['startLine']);
	 ...
	 * @throws \TYPO3\PackageBuilder\Exception\ParseError
	 * @return void
	 */
	protected function onMethodFound($methodMatches) {
		...
		try {
			...
			if ($this->currentMethod['reflection']) {
				$classMethod = new \TYPO3\PackageBuilder\Domain\Model\ClassObject\Method($methodName, $this->currentMethod['reflection']);
				...
				throw new \TYPO3\PackageBuilder\Exception\ParseError((((('Method ' . $methodName) . ' does not exist. Parsed from line ') . $this->lineCount) . 'in ') . $this->classReflection->getFileName());
			}
		} catch (ReflectionException $e) {
			// ReflectionClass throws an exception if a method was not found
			\t3lib_div::devlog('Exception: ' . $e->getMessage(), 'extension_builder', 2);
		}
	}
	...
	protected function addConstant($constantMatches) {
		for ($i = 0; $i < count($constantMatches[0]); $i++) {
			try {
				...
				\t3lib_div::devlog((((('Exception in line : ' . $e->getMessage()) . ' Constant ') . $constantName) . ' found in line ') . $this->lineCount, 'extension_builder');
			}
		}
	}
		...
		foreach ($properties as $propertyName => $propertyValue) {
			try {
				...
				if ($reflectionProperty) {
					$classProperty = new \TYPO3\PackageBuilder\Domain\Model\ClassObject\Property($propertyName);
					...
					throw new \TYPO3\PackageBuilder\Exception\ParseError(((((' Property ' . $propertyName) . ' does not exist. Parsed from line ') . $this->lineCount) . 'in ') . $this->classReflection->getFileName());
				}
			} catch (ReflectionException $e) {
				// ReflectionClass throws an exception if a property was not found
				\t3lib_div::devlog((((('Exception in line : ' . $e->getMessage()) . 'Property ') . $propertyName) . ' found in line ') . $this->lineCount, 'extension_builder');
			}
		}
	}
