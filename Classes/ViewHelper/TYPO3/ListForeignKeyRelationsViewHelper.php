 ...
 * @scope prototype
 */
class ListForeignKeyRelationsViewHelper extends \TYPO3\Fluid\Core\ViewHelper\AbstractViewHelper {
	...
	public function render($extension, $domainObject) {
		...
		foreach ($extension->getDomainObjects() as $domainObject) {
			...
			foreach ($domainObject->getProperties() as $property) {
				if ($property instanceof \TYPO3\PackageBuilder\Domain\Model\DomainObject\Relation\ZeroToManyRelation
					&& $property->getForeignClassName() === $expectedDomainObject->getClassName()
				) {
					$results[] = $property;
				}
			}
