...
<f:for each="{domainObject.properties}" as="property">
	'{property.fieldName}' => array(
		...
		'config' => array(
			<pb:format.indent indentation="3"><f:render partial="TCA/{property.dataType}.phpt" arguments="{property: property,extension:domainObject.extension,settings:settings}" /></pb:format.indent>
		),<f:if condition="{property.useRTE}">
		'defaultExtras' => 'richtext[]',</f:if>
	),</f:for>
);
<f:for each="{pb:typo3.listForeignKeyRelations(extension: domainObject.extension, domainObject: domainObject)}" as="relation">
...
<pb:recordType domainObject="{domainObject}">
